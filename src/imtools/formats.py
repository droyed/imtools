...
def to_pil_image(source):
    """
    Convert various image sources to a PIL Image.
    
    Args:
        source: Image source, can be one of:
...
def to_numpy_image(
    ...
    *,
    ...
    """
    Loads image as RGB/RGBA (NumPy Standard) with optional normalization.
    """
    img = None
...
def imwrite(image_arr, file_path, input_channel_order='RGB', quality=85, optimize=True):
    """
    Saves a NumPy array (uint8, uint16, float, bool) to disk.
    Handles OpenCV (BGR/BGRA) inputs automatically via input_channel_order.
    """
